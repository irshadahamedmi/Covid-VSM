...
$query="update tblmoh set FullName='$fname',MobileNumber='$mnumber',Email='$email',NIC='$nic',Area='$area',District='$district' where id='$pid'";
$result =mysqli_query($con, $query);
if ($result) {
echo '<script>alert("Moh record updated successfully.")</script>';
  echo "<script>window.location.href='manage-moh.php'</script>";
} 
else {
...
<head>
    ...
    <title>Vaccination Management System | Edit Moh</title>
...
<?php 
$pid=intval($_GET['pid']);
$query=mysqli_query($con,"select * from tblmoh where id='$pid'");
$cnt=1;
while($row=mysqli_fetch_array($query)){
?>                 
<h1 class="h3 mb-4 text-gray-800"><?php echo $row['FullName'];?>'s Profile</h1>
<form name="editmoh" method="post">
  <div class="row">
                        ...
                        <div class="form-group">
                            <label>Full Name</label>
                                  ...
                                  <input type="text" class="form-control" id="mobilenumber" name="mobilenumber" placeholder="Please enter your mobile number" pattern="[0-9]{10}" title="10 numeric characters only" value="<?php echo $row['MobileNumber'];?>" required="true" >
									...
									<div class="form-group">
										<label>Area</label>
														<input type="text" class="form-control" id="area" name="area"  placeholder="Enter your Area..." pattern="[A-Za-z ]+" title="letters only"  required="true">
															</div>	
